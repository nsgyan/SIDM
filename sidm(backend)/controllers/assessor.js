...
exports.assessmentsList=(req,res)=>{
  ...
  registrationForm.find({questionnaireStatus:'Submitted'}).then(data=>{
    res.status(200).send(data)
  }).catch(err=>{
      res.status(500).json("internal server error");
  })
...
exports.findmember=(req,res)=>{
  ...
  RegistrationForm.find({category:category,typeOfApplicant:typeOfApplicant,questionnaireStatus:'Submitted' , assessor : { $elemMatch: {  email : { $gte: asaessorEmail }, status:{ $gte: status }}, } }).then(data=>{
      res.status(200).send(data)
  }).catch(err=>{
      res.status(500).json("internal server error");
  })
