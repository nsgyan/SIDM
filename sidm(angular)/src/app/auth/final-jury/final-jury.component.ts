...
@Component({
  selector: 'app-final-jury',
  templateUrl: './final-jury.component.html',
  styleUrls: ['./final-jury.component.css']
})
export class FinalJuryComponent implements OnInit {
    ...
    if (this.assessorform.valid) {
      ...
      this.httpService.assessorLogin({ email: this.assessorform.value.email, password: this.assessorform.value.password })
        .subscribe((data: any) => {
         ...
         this.router.navigate(['/finalJury'])
        
      //   this.getassessorData()
      //    const url='/dashboard/assessor/view/'+data.data._id
      //    this.routes.navigateByUrl(url);
      //  window.location.href=url
        this.toast.success('Final Jury Successfully login!');
        }, err => {
